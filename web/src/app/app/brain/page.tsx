...
type Personalization = { memory: string; soul: string; agent_name: string };
...
export default function BrainPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [threadsRes, pRes] = await Promise.all([
          fetch("/api/chat/threads", { cache: "no-store" }),
          fetch("/koraku-api/api/personalization", {
            cache: "no-store",
            credentials: "include",
          }),
        ]);
        ...
        if (pRes.ok) {
          setPersonalization((await pRes.json()) as Personalization);
        }
      } catch (e) {
    ...
    <main className="min-h-0 flex-1 overflow-y-auto bg-white px-6 py-10">
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm font-medium leading-relaxed text-neutral-600">
              A lightweight beta library for saved notes, memories, recent chats, and
              workspace artifacts. Use chat to create files, then open the Workspace panel
        ...
        <input
          ...
          placeholder="Search notes, memories, and chats..."
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <aside className="space-y-5">
            <section className="rounded-[32px] bg-neutral-950 p-6 text-white">
              <h2 className="text-lg font-bold">Durable memory</h2>
              <div className="mt-4 flex flex-wrap gap-2">
                {memoryItems.length > 0 ? (
                  memoryItems.slice(0, 14).map((item) => (
                  ...
                  <p className="text-sm font-medium text-white/60">
                    No memory yet. Add preferences in Personalization or onboarding.
                  </p>
                )}
              </div>
            </section>
