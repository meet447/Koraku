...
from koraku.integrations.runtime_user import effective_runtime_user_id, reset_runtime_user_id, set_runtime_user_id
...
def test_session_workspace_differs_with_and_without_tenant_org() -> None:
    ...
    cloud_tok = set_runtime_user_id(user)
    org_tok = set_tenant_org_id(org)
    try:
        with_org = session_workspace_root_posix(effective_runtime_user_id(), sid, settings)
    finally:
        reset_tenant_org_id(org_tok)
        reset_runtime_user_id(cloud_tok)

    cloud_tok2 = set_runtime_user_id(user)
    try:
        without_org = session_workspace_root_posix(effective_runtime_user_id(), sid, settings)
    finally:
        reset_runtime_user_id(cloud_tok2)
